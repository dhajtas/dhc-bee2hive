...
//	const prog_int16_t *window = tbl_window;
	...
	//  urobit kalibraciu 0 ako jedno cele meranie a vypocitat priemernu hodnotu - ulozit do eeprom?
	//	v = ADCA.CH0RES - 0x0874;	// 0V = 0x0C8... 1/2 je o cca 200dec vyssie ako 0x7FF
	v = ADCA.CH0RES - 0x0874;
	vv = ADCA.CH1RES - 0x0874;				// 0V = 0x0C8... 1/2 je o cca 200dec vyssie ako 0x7FF
	Signal[0][adc_i] += v;
	Signal[1][adc_i] += vv;
	//	PORTD.OUTTGL = _BV(LED0);		//sampling frequency test
	//	65kHz sampling frequency; with clk div by 2, sampling frequenci 32.4kHz...
...
*/
	ADCA.CTRLA = ADC_ENABLE_bm | ADC_FLUSH_bm;	// enable ADC, no DMA control, no conversion start, flush pipeline 
	ADCA.CTRLB = ADC_IMPMODE_LOWIMP_gc | ADC_CURRLIMIT_NO_gc | ADC_RESOLUTION_12BIT_gc;		// free-running at this time?
	ADCA.REFCTRL = ADC_REFSEL_VCC_gc | ADC_BANDGAP_bm;		// enable band-gap, reference = VCC/1.6
	ADCA.PRESCALER = ADC_PRESCALER_DIV256_gc;				// clk/512 - 31.25kSps @ 16MHz; clk/256 - 125kSps @ 32MHz, decimated by 2 - 13bit @ 31.25kSps
	ADCA.EVCTRL = ADC_SWEEP_01_gc;							// free-running channels 0 1
//	ADCA.EVCTRL = ADC_SWEEP_0_gc;							// free-running channel 0 
